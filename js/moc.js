...
const createObject = () =>  {
  ...
  const offerPrice = randomNumber(10, 1000);
  const offerType = TYPES[randomNumber(0, TYPES.length-1)];
  const roomsNumber = randomNumber(1,10);
  ...
  const randomPhotos = PHOTOMOCS[randomNumber(0, PHOTOMOCS.length-1)];
